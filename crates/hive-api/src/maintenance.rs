...
pub(crate) async fn run(pool: PgPool, state: ApprovalMaintenanceState) {
    let mut interval = tokio::time::interval(Duration::from_secs(1));
    interval.set_missed_tick_behavior(MissedTickBehavior::Skip);
    loop {
        interval.tick().await;
        deployment::reconcile_approval_expiry(&pool, &state).await;
        deployment::reconcile_approval_upgrade(&pool, &state).await;
    }
}
