//! `AgentDraftEditorService`, which canonicalizes route identifiers before the repository
//! applies visibility and current write authority. Reads are not here: drafts, versions, the
//! review and the version comparison are generated entity reads with computed fields.

use crate::agent::models::{AgentDraftMutationProblem, AgentDraftMutationResult};
use crate::agent::repository::{AgentDraftRepository, CommandResult};
use uuid::Uuid;
...
mod tests {
    use super::*;
    use crate::agent::models::AgentDraftProblemKind;
