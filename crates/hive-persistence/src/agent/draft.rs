...
impl AgentDraftRepository for PgAgentDraftRepository {
    ...
    async fn versions(
        ...
        let id_rows = db.query_all_raw(statement).await.map_err(other)?;
        let mut values = Vec::with_capacity(id_rows.len());
        for row in id_rows {
            let id: Uuid = row.try_get_by("id").map_err(other)?;
            if let Some(version) = version_row(db, agent, id).await.map_err(other)? {
                values.push(version);
            }
        }
        Ok(Some(values))
    ...
    async fn version(
        ...
        version_row(db, agent, version_id).await.map_err(other)
    }
